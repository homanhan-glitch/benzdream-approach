...
(() => {
 ...
 document.addEventListener('click',event=>{
  ...
  if(!method)return;
  seen.add(event);
  emit({send_to:measurement,contact_method:method,page_path:location.pathname,link_url:method==='phone'?'tel:':url.origin+url.pathname,button_location:link.closest('header,.bd-site-header,.site-header')?'header':link.closest('footer,.bd-site-footer,.site-footer')?'footer':'body',transport_type:'beacon'});
 },true);
})();
